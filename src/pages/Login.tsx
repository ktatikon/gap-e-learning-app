...
import { signInWithPassword, getRoleBasedRoute } from "@/lib/auth";
...
export default function Login() {
  ...
  React.useEffect(() => {
    if (user) {
      navigate(getRoleBasedRoute(user.roles));
    }
  }, [user, navigate]);
  ...
  const handleAutoLogin = async (
    ...
    try {
      const user = await signInWithPassword(userEmail, userPassword);
      ...
      toast({
        ...
        description: error.message || "Failed to login automatically",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
