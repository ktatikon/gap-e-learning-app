...
import { Shield, AlertTriangle, CheckCircle2, TrendingUp, Users, FileCheck } from 'lucide-react';
...
export default function ComplianceDashboard() {
  ...
  const completedProgress = userProgress.filter(p => p.status === 'completed').length;
  ...
  const failedCases = [
    { user: 'John Doe', module: 'GMP Fundamentals', attempts: 4, lastAttempt: '2024-01-15' },
    { user: 'Jane Smith', module: 'Deviations & CAPA', attempts: 3, lastAttempt: '2024-01-14' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="gxp-card">
        ...
        <p className="text-muted-foreground">
          Monitor training compliance, audit activities, and ensure regulatory requirements are met.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{overdueRate}%</div>
            <p className="text-xs text-muted-foreground">
            ...
            {departmentData.map((dept) => (
              <div key={dept.name} className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{dept.completion}% ({dept.total} users)</span>
                </div>
              ...
              {failedCases.map((case_, index) => (
                <div key={index} className="p-3 border rounded-lg">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-medium text-foreground">{case_.user}</h3>
                    <span className="text-xs text-red-600 font-medium">{case_.attempts} attempts</span>
                  </div>
                  <p className="text-sm text-muted-foreground">{case_.module}</p>
                  <p className="text-xs text-muted-foreground">Last attempt: {case_.lastAttempt}</p>
                </div>
              ))}
            </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent>
            <div className="space-y-3">
              {auditLogs.slice(0, 5).map((log) => (
                <div key={log.id} className="grid grid-cols-4 gap-2 text-xs border-b pb-2">
              ...
              {auditLogs.length === 0 && (
                <p className="text-center text-muted-foreground py-4">No audit activity yet</p>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Daily compliance validation tasks
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
